...
## [0.0.5] — 2026-05-17 (both PyPI + npm — version-sync release)
...
- **Versions now sync across PyPI and npm.** `lumo-mobile` (PyPI) and
  ...
  changed code. Easier to reason about, easier to support.
- **`@onexeor/lumo 0.0.6` ships a fresh skill bundle** containing
  `lumo-source` for both Compose and SwiftUI. The 0.0.5 npm tarball
  shipped a stale SKILL.md from v0.0.1 — every `npx @onexeor/lumo init`
  was installing the old skill without the source-AST tool. The
  installer/skill bundle is regenerated by
  `installer/scripts/bundle-skill.js` before each release; missing that
  step here was the root cause.
...
- PyPI 0.0.5 is a **version-sync release** — the Python code is
  identical to 0.0.4. No new behaviour, no new tools, no new
  dependencies. Only `version` strings change.
