...
|---|---|---|---|
| 1 | Markdown source + LLM round-trip | `feat/spec-check-markdown` | Implemented |
...
| 5 | Representative dogfood + golden cases + docs | `feat/spec-check-dogfood` | Draft |
...
lumo-spec check --layout screen.json --source jira --issue-key APP-1234
...
- **Spec length cap** — derived from model context + real spec
...
3. Dogfood: one APP, one UI, one MOBILE ticket per release after
   Phase 5 lands.
