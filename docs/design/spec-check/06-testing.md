...
Gated behind `LUMO_TEST_DOGFOOD=1`. Use a small representative set of
Jira tickets per release. Run `lumo-figma render` + `lumo-spec check
--source jira --issue-key …` on each. Compare output across releases to
catch regressions.
...
- Live run nightly with `LUMO_TEST_LIVE=1` and a repository secret
