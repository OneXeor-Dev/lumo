...
3. Storage-format → markdown converter is a separate module
   ...
   implemented and tested inside Lumo rather than imported from any
...
Comments included because many teams refine spec details in the
comment thread.
...
- Multi-ticket aggregation needs a merge strategy (which sub-task
  wins on conflict?).
- Mobile teams often keep the full spec in the
...
This is deliberately implemented inside Lumo rather than depending on
project-specific helper code. The converter should be tested against
Lumo's own fixtures rather than assuming another project's parser is
available or well covered.

### Node types to handle

The table below lists the node types worth handling. The actual set
real specs use should be confirmed by sampling real Confluence / Jira
