...
- `tools/lumo/spec/storage_format.py` — only if representative dogfood needs
  v1 fallback.
...
**Scope:**
- A small representative set of Jira tickets. Capture output,
...
- `lumo-spec check --layout … --source jira --issue-key APP-…`
  works against a real Jira ticket.
- `lumo-spec check --layout … --source confluence --page-id …`
  works against a real Confluence page.
