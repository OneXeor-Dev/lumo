...
def _srgb_to_oklab(r: int, g: int, b: int) -> tuple[float, float, float]:
    ...
    l_channel = 0.4122214708 * rl + 0.5363325363 * gl + 0.0514459929 * bl
    m = 0.2119034982 * rl + 0.6806995451 * gl + 0.1073969566 * bl
    s = 0.0883024619 * rl + 0.2817188376 * gl + 0.6299787005 * bl

    l_, m_, s_ = (math.copysign(abs(v) ** (1 / 3), v) for v in (l_channel, m, s))
...
def _oklab_to_linear_srgb(L: float, a: float, b: float) -> tuple[float, float, float]:
    ...
    return (
        4.0767416621 * l_channel - 3.3077115913 * m + 0.2309699292 * s,
        -1.2684380046 * l_channel + 2.6097574011 * m - 0.3413193965 * s,
        -0.0041960863 * l_channel - 0.7034186147 * m + 1.7076147010 * s,
    )
